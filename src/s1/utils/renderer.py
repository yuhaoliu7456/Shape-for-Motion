...
def render_depth(
    ...
    rast_out, rast_deriv = dr.rasterize(glctx, v_pos_clip, mesh_t_pos_idx, resolution=resolution)

    R = pose[:3, :3]
    R_inv = R.transpose(0, 1)
    mesh_v_pose_rot = mesh_v_pos @ R_inv
    depth = mesh_v_pose_rot[..., 2:3].contiguous()  # [N, 1]
    output, _ = dr.interpolate(depth, rast_out, mesh_t_pos_idx)
    output = dr.antialias(output, rast_out, v_pos_clip, mesh_t_pos_idx)
    output = torch.flip(output, dims=[1])[0]
...
def mesh_renderer(
    ...
    if viewpoint_cam is not None:
        ...
        return mask, mesh_image, depth, verts, faces, vtx_color
    else:
        return verts, faces, vtx_color


def updated_mesh_renderer(
    glctx: dr.RasterizeCudaContext,
    verts=None,
    faces=None,
    vtx_color=None,
    ...
    verts_d=None,
    faces_d=None,
    new_obj_verts=None, new_obj_faces=None,
    verts_ori=None, faces_ori=None,
    scale_focal_length=1.0
):
    ...
    if viewpoint_cam.K is not None:
        ...
        focalx = fov2focal(fovx, viewpoint_cam.image_width)
        focaly = fov2focal(fovy, viewpoint_cam.image_height)
        focalx = focalx * scale_focal_length
        focaly = focaly * scale_focal_length
    ...
    depth = render_depth(
        glctx,
        verts if verts_d is None else verts_d,
        faces if faces_d is None else faces_d,
        pose,
        K,
        resolution=[viewpoint_cam.image_height, viewpoint_cam.image_width],
    )
    ...
    if new_obj_verts is not None and new_obj_faces is not None:
        mask_of_new_obj = render_mask(
            glctx,
            new_obj_verts,
            new_obj_faces,
            pose,
        ...
        mask_of_new_obj = mask_of_new_obj[..., [0]]
    else:
        mask_of_new_obj = torch.zeros(mask.shape)

    if verts_ori is not None and faces_ori is not None:
        mask_ori_obj = render_mask(
            glctx,
            verts_ori,
            faces_ori,
            pose,
        ...
        mask_ori_obj = mask_ori_obj[..., [0]]
    else:
        mask_ori_obj = torch.zeros(mask.shape)
    ...
    return mask, mesh_image, depth, mask_of_new_obj, mask_ori_obj
