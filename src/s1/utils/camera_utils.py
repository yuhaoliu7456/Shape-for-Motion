...
import numpy as np
from s1.utils.general_utils import PILtoTorch, ArrayToTorch
from s1.utils.graphics_utils import fov2focal
from s1.nvdiffrast_utils.util import opencv2blender
import json
import torch
...
def loadCam(args, id, cam_info, resolution_scale):
    ...
    if cam_info.depth is not None:
        loaded_depth = torch.tensor(cam_info.depth)
    return Camera(colmap_id=cam_info.uid, R=cam_info.R, T=cam_info.T,
                  FoVx=cam_info.FovX, FoVy=cam_info.FovY,
                  image=gt_image, gt_alpha_mask=loaded_mask, gt_depth=loaded_depth, 
                  image_name=cam_info.image_name, image_path=cam_info.image_path, uid=id, orig_transform=cam_info.orig_transform, orig_img=cam_info.orig_img,
                  ...
                  vid=cam_info.vid, tid=cam_info.tid)
