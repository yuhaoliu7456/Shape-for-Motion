...
class SAVDataset:
    ...
    def read_frames(self, mp4_path: str) -> None:
        ...
        """
        if not smart_exists(mp4_path):
            print(f"{mp4_path} doesn't exist.")
            return None
        else:
            # decode the video
            # frames = decode_video(mp4_path)
            video_reader = VideoReader(smart_open(mp4_path, 'rb'), ctx=cpu(0))
            frames = video_reader.get_batch(np.arange(0, len(video_reader))).asnumpy()
    ...
    def visualize_annotation(
        ...
        if len(rles) > 0:
            ...
            combined_mask = np.zeros((masks[0].shape[0], masks[0].shape[1]))
            combined_mask = np.zeros((masks[0].shape[0], masks[0].shape[1], 3))
            for i, mask in enumerate(masks):
                # 为每个通道分别赋值
                combined_mask[mask] = colors[i].reshape(1, 3)
            # combined_mask = np.stack([combined_mask] * 3, axis=-1)
            # cv2 RGB2BGR
            combined_frame = np.concatenate([cv2.cvtColor(frames[annotated_frame_id], cv2.COLOR_RGB2BGR), combined_mask* 255], axis=1)
            cv2.imwrite("combined_mask.png", combined_frame )
