...
def preprocess_video(input_path, remove_bg=False, n_frames=21, W=576, H=576, output_folder=None, image_frame_ratio = 0.917):
    ...
    for idx, image in enumerate(images):
        ...
        # meanwhile, save the images
        # save_numpy_img(os.path.join(output_folder, 'processed_frames', os.path.basename(all_img_paths[idx])), image)
...
def sample_sv3d(
    ...
    with torch.no_grad():
        with torch.autocast(device):
            ...
            # [21, 4, 72, 72], 这里可以用sv4d生成的8个views来填充initial noise (SDEdit)
            # define vae, 读取8个views的latent
            # 在这8个views上加noise，然后替换randn中原始8个noise
...
def prepare_inputs(frame_indices, img_matrix, v0, view_indices, model, version_dict, seed, polars, azims):
    ...
    cond_motion = torch.cat([img_matrix[t][v0] for t in forward_frame_indices], 0) # [第v0列], frame的cond是each view的1st frame
    cond_view = torch.cat([img_matrix[t0][v] for v in view_indices], 0) # [第t0行], view的cond是reference video的所有frame
    ...
    t0 = backward_frame_indices[0] # backward的时候只有t做了reverse,v0和view_indices都是一样的
    image = img_matrix[t0][v0]
...
def load_model(
    ...
    # modify the ckpt_path in config 
